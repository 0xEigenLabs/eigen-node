    ...
    mod babyjubjub {
        ...
        proptest! {
            ...
            fn or_babyjubjub(
                ...
                Y in babyjubjub_point(),
            ) {
                let xG = G().mul_scalar(&x);
                type OrDL = Or<babyjubjub::DLG<U31>, babyjubjub::DLG<U31>>;
                let statement = (xG.clone(), Y.clone());
                let proof_system = crate::FiatShamir::<OrDL, HashTranscript<Hasher,ChaCha20Rng>>::default();
                ...
                assert!((xG.equals(&Y)) || !proof_system.verify(&statement, &wrong_proof_lhs));
