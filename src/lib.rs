...
impl Signature {
    ...
    pub fn verify(&self, label: &'static [u8], message: &[u8], pubkey: &CompressedRistretto) -> bool {
        let mut transript = Transcript::new(b"sign");
        transript.domain_sep();
        transript.append_message(label, message);
        let c = {
        ...
        let result = RistrettoPoint::optional_multiscalar_mul(
            iter::once(-self.s).chain(
                iter::once(Scalar::one()).chain(iter::once(c))),
            iter::once(Some(RISTRETTO_BASEPOINT_POINT)).chain(
                iter::once(self.R.decompress()).chain(
                    iter::once(pubkey.decompress()))
            ),
        ).unwrap();
